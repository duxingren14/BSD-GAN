...
class instance_batch_norm(object):
  def __init__(self, name="instance_batch_norm"):
    self.name = name
    self.eps = 1e-6
...
def celoss(logits, labels):
  if not isinstance(labels, (np.ndarray, list, tuple)):
      labels = labels * np.ones(shape = tuple(logits.shape), dtype = np.float32)
